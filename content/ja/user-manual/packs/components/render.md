...
レンダリングコンポーネントは、エンティティがプリミティブ形状またはレンダリングアセットをレンダリングするのを可能にします。

レンダリングコンポーネントは、コンポーネントパネルの右上にあるトグルを使用して有効または無効にできます。有効にされている場合、コンポーネントで参照されるメッシュインスタンスがシーンに追加され、レンダリングされます。
...
| Asset                 | このレンダリングコンポーネントによってレンダリングされるレンダリングアセット。タイププロパティが「Asset」に設定されている場合にのみ適用されます。レンダリングコンポーネントには、単一のレンダリングアセットしか割り当てられません。 |
...
| Static                | このレンダリングコンポーネントを参照するエンティティが移動しない場合は、エンジンに最適化を行うヒントとして、このボックスをチェックします。 |
...
[Scriptコンポーネント][2]を使用して、レンダリングコンポーネントのプロパティを制御できます。レンダリングコンポーネントのスクリプトインタフェースは[こちら][3]です。
