...
設定パネルを使用すると、さまざまなプロパティを設定できます。エディターの左下にある`cog`ボタン([ツールバー][1])を使用してアクセスできます。
...
エディターカメラの近くのCLIIP平面と遠くのCLIIP平面を設定できます。
...
Launch Localでアプリケーションを起動するときにすべてのスクリプトが読み込まれるURLです。

#### Locale

エディターでプレビューすることができるロケールと、アプリケーションを起動するときにプレビューすることができるロケールです。これは、テキスト要素のローカライズが有効になっている場合や、ローカライズされたアセットを使用する場合に使用されます。
