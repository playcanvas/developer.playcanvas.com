...
## Управление активами в сценах

Частый вопрос по сценам заключается в том, будут ли активы, используемые в сцене, загружаться вместе с загрузкой сцены. В PlayCanvas активы и сцены разделены и должны быть загружены отдельно, что дает разработчику большую степень гибкости.

Рекомендуется помечать все активы с именем сцены, необходимым в сцене, и при загрузке сцены сначала загружать активы, а затем, когда все активы загружены, начинать загрузку сцены.

Более подробную информацию о тегах ассетов и загрузке ассетов можно найти на [этой странице][asset-tags-loading].

[Пример проекта][asset-load-for-scene-project] ниже загружает активы при загрузке сцены и выгружает их при возврате в главное меню.
