...
In this tutorial we will show you how to use forces to control a dynamic rigidbody. We will briefly show the use of forces, impulses, torques and the use of Designer settings to customize behaviour (via gravity, damping and friction settings).

##Moving dynamic rigidbodies

The first point to note is that *you should not set velocities* on dynamic rigidbody types, code dealing with velocity such as `this.entity.rigidbody.linearVelocity.x` is read-only for dynamic bodies. However, you can, as usual, read and set the Entity’s position via [Entity methods][entities], however the rigidbody must then by synced to the transform matrix of the parent Entity via the `SyncEntityToBody()` command.
...
~~~
When the `brakeY` flag has not been activated (see Limiting and Control section below) the cube is given a y-axis impulse to impart an instant change of velocity.
...
~~~
Torques are applied to the entity via `.rigidbody.applyTorque(x, y, z)`.

###TorqueImpulses
~~~javascript~~~
this.entity.rigidbody.applyTorqueImpulse(x, y, z)
~~~
Instantaneous changes in angular velocity are applied via `applyTorqueImpulse(x, y, z)`. This was not used in the code for the above demo.


##Limiting and control

Both Designer settings and code were set to prevent the constant application of unbalanced forces (and so prevent a body from continuously accelerating and moving out of control). We enabled angular damping on the cube’s attribute editor as well as friction on both the cube and floor.
...
teleport: function (newXPosition) {
    this.entity.setLocalPosition(newXPosition, this.playerPos.y, this.playerPos.z);
...
pc.script.create('DynamicBody', function (context) {
    ...
    DynamicBody.prototype = {
        ...
        update: function (dt) {
            ...
            if (playerVel.y > this.maxVelocityY || this.playerPos.y > 8) {
                this.entity.rigidbody.applyImpulse(0, -0.1 * playerVel.y, 0);
                this.brakeY = true;
            } else {
                this.brakeY = false;
            }
