...
Changing the `width` makes the fill image larger and changing the `rect` makes sure that we only show the portion of the texture that is visible, so that we avoid stretching the visible texture. Here is the API reference for `rect` /api/pc.ElementComponent.html#rect.
