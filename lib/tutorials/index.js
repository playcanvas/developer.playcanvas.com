...
const getTags = async (done) => {
    const response = await fetch('https://playcanvas.com/api/tags');
    if (!response.ok) {
        throw new Error(`failed to download tags (${response.status})`);
    }
    const json = await response.json();
    const tags = json.result
        .filter(tag => tag.kind === 'app')
        .filter(tag => tag.id !== 'challenge' && tag.id !== 'skype' && tag.id !== 'tutorial');
    done(tags);
};

module.exports = function makePlugin(dir) {
    // compile the template used to generate pages for tagged projects
    handlebars.registerPartial("template-tagged-project", fs.readFileSync("templates/partials/tutorial-tagged-project.hbs", "utf-8"));
        ...
        return function (files, metalsmith, done) {
            ...
            for (let i = 0; i < langs.length; i++) {
                ...
                for (const filepath in files) {
                    ...
                    if (filepath.startsWith(path.join(lang, dir))) {
                        ...
                        if (filepath.endsWith(path.join('tutorials', 'index.html'))) continue;
            ...
            getTags(function (tags) {
                getTaggedProjects(function (tagged) {
                    ...
                    handlebars.registerPartial(contentsTemplatePartialName, fs.readFileSync("templates/partials/tutorial-contents.hbs", "utf-8"));
                    ...
                    handlebars.registerPartial(contentsLinksTemplatePartialName, fs.readFileSync("templates/partials/tutorial-contents-links.hbs", "utf-8"));
                    ...
                    handlebars.registerPartial(listTemplatePartialName, fs.readFileSync("templates/partials/tutorial-list.hbs", "utf-8"));
