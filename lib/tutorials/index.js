    ...
    return function (opts) {
        return function (files, metalsmith, done) {
            ...
            for (let i = 0; i < langs.length; i++) {
                ...
                for (const filepath in files) {
                    ...
                    if (filepath.startsWith(path.join(lang, dir))) {
                        ...
                        // Add tutorial tags to list (only for english)
                        if (lang == "en") {
            ...
            getTaggedProjects(function (tagged) {
                ...
                const unusedTutorialTags = new Set(['monetization', 'tutorial', 'material']);
                tutorialTags = [...tutorialTags].filter(tag => !unusedTutorialTags.has(tag));

                const context = {
                    pages: {},
                    tags: Array.from(tutorialTags).sort()
                };
